...
function Show-Menu {
    ...
    Write-Host "1️⃣  Deletar Pod (Auto-Healing)" -ForegroundColor Yellow
    Write-Host "2️⃣  Stress CPU (HPA)" -ForegroundColor Yellow  
    Write-Host "3️⃣  Status Atual" -ForegroundColor Yellow
    Write-Host "4️⃣  Escalar Manualmente" -ForegroundColor Yellow
    Write-Host "5️⃣  Reset Completo" -ForegroundColor Yellow
...
function Show-Status {
    ...
    Write-Host "🏗️  Deployments:" -ForegroundColor Cyan
    ...
    kubectl top pods -l app=patocast-backend,app=patocast-frontend 2>$null || Write-Host "⚠️  Metrics server pode não estar disponível" -ForegroundColor Yellow
}

# Função para escalar manualmente
function Scale-Manual {
    Write-Host "⚖️  ESCALAR MANUALMENTE" -ForegroundColor Magenta
